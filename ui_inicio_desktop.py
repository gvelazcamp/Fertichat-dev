...
def mostrar_inicio_desktop():
    ...
    with st.sidebar:
        ...
        if st.button("▸ Baja de stock", key="sidebar_baja"):
            ...
            st.rerun()
        if st.button("▸ Órdenes compra", key="sidebar_ordenes"):
    ...
    st.markdown(f"<div style='text-align:center;margin:50px auto 20px auto;font-size:24px;color:#1e293b;'>{saludo}, {nombre}!</div>", unsafe_allow_html=True)
