...
def _cache_proveedores() -> list:
    ...
    try:
        # SQL directo como en compras_ia.py - trae TODOS sin límite
        res = supabase.rpc('get_proveedores_unicos').execute()
        
        # Si no existe la función RPC, usar select normal
        if not res.data:
            # Traer todos los registros y extraer proveedores únicos
            out = []
            start = 0
            page = 1000
            max_rows = 100000  # Aumentado para traer todos
            ...
            for r in out:
                # Buscar campo que contenga "proveedor" o "cliente"
            ...
            result = sorted(list(proveedores))
            print(f"🐛 DEBUG: Cargados {len(result)} proveedores únicos")
            return result
        
        return [r.get('prov', '') for r in res.data if r.get('prov')]
        
    except Exception as e:
        # Fallback: intentar con select directo del campo
        try:
            print(f"⚠️ Intentando fallback para proveedores: {e}")
            ...
            result = sorted(list(proveedores))
            print(f"🐛 DEBUG fallback: Cargados {len(result)} proveedores únicos")
            return result
            
        except Exception as e2:
            st.error(f"Error cargando proveedores: {e2}")
            return []
