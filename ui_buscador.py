# =========================
# UI_BUSCADOR.PY - MÓDULO BUSCADOR IA FINAL
# =========================
    ...
    if intencion == 'ultima_factura':
        if art_clean:
            ...
            sql = f"""
                ...
                LIMIT 1
            """
            df = ejecutar_consulta(sql, (f"%{art_clean}%",))
            ...
            sql = f"""
                ...
                LIMIT 1
            """
            df = ejecutar_consulta(sql, (f"%{prov_clean}%",))
            if df is not None and not df.empty:
...
def mostrar_filtros_rapidos():
    ...
    # Fila 2: Fechas, búsqueda y botón
    col4, col5, col6, col7 = st.columns([1.8, 1.8, 2.2, 0.8])
...
def mostrar_panel_ia():
    """Panel colapsable de consulta a IA en sidebar."""
    with st.sidebar:
        st.markdown("---")
...
def mostrar_buscador_ia():
    """Pantalla principal del Buscador de Comprobantes."""
