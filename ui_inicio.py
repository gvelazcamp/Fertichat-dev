...
def mostrar_inicio():
    ...
    /* =========================
       DESKTOP: OCULTAR MOBILE, MOSTRAR DESKTOP
       ========================= */
    ...
    /* Grid para tarjetas desktop */
    div[data-testid="stAppViewContainer"]:has(#fc-home-marker) .fc-dgrid{
        display: grid;
    ...
    /* Tarjeta desktop (clickeable) */
    div[data-testid="stAppViewContainer"]:has(#fc-home-marker) .fc-dcard{
        display: flex;
        align-items: center;
        gap: 14px;
        width: 100%;
    ...
    div[data-testid="stAppViewContainer"]:has(#fc-home-marker) .fc-dcard:first-line{
        font-size: 16px;
        font-weight: 900;
        color: #0f172a;
    ...
    div[data-testid="stAppViewContainer"]:has(#fc-home-marker) .fc-dcard:hover{
    ...
    /* Ícono desktop */
    div[data-testid="stAppViewContainer"]:has(#fc-home-marker) .fc-dicon{
        width: 54px;
        height: 54px;
        border-radius: 16px;
        display: flex;
    ...
    # =========================
    # Helpers HTML (desktop y móvil)
    # =========================
    def _dcard(go: str, icon: str, title: str, sub: str, tile_class: str) -> str:
        return f'''
        <div class="fc-dcard" onclick="const url = new URL(window.location); url.searchParams.set('go', '{go}'); window.location.href = url.toString();">
            <div class="fc-dicon {tile_class}">{icon}</div>
            {title}\n{sub}
        </div>
        '''
    ...
    # =========================
    # LAYOUT DESKTOP (NUEVO - TARJETAS HTML EN GRID)
    # =========================
    ...
    st.markdown(
        f"""
        <div class="fc-dgrid">
            {_dcard("compras", "🛒", "Compras IA", "Consultas inteligentes", "tile-compras")}
            {_dcard("buscador", "🔎", "Buscador IA", "Buscar facturas / lotes", "tile-buscador")}
            {_dcard("stock", "📦", "Stock IA", "Consultar inventario", "tile-stock")}
            {_dcard("dashboard", "📊", "Dashboard", "Ver estadísticas", "tile-dashboard")}
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        f"""
        <div class="fc-dgrid">
            {_dcard("pedidos", "📄", "Pedidos internos", "Gestionar pedidos", "tile-pedidos")}
            {_dcard("baja", "🧾", "Baja de stock", "Registrar bajas", "tile-baja")}
            {_dcard("ordenes", "📦", "Órdenes de compra", "Crear órdenes", "tile-ordenes")}
            {_dcard("indicadores", "📈", "Indicadores", "Power BI", "tile-indicadores")}
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    # =========================
    # LAYOUT MÓVIL (IGUAL QUE ANTES - NO TOCADO)
    # =========================
